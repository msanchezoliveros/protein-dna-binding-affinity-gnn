...
1. **Node encoder**: Linear projection → LayerNorm → GELU → Dropout ($d_h = 24$ channels).
...
3. **Edge encoder**: Two-layer perceptron per relationship type, projected to a shared width ($d_e = 16$ channels).
4. **ESM-2 encoder**: Projection of the evolutionary embeddings ($1280 \rightarrow 16$ channels), controlled integration during training.
5. **Global context encoder**: Two-layer perceptron ($96 \rightarrow 8$ channels).
6. **Regression head**: Concatenation of the four resulting vectors (72 dimensions) → LayerNorm → GELU → Dropout → linear projection to a single scalar value ($\mathrm{p}K_{\mathrm{d}}$).

### Training
- **Cross-validation**: 5-fold scheme, stratified by quantiles of experimental $\mathrm{p}K_{\mathrm{d}}$. Per fold: 313 training graphs, 79 validation graphs, 99 test graphs.
- **Normalization**: Per-column standardization (robust scaling if $|\text{skewness}| > 1.0$, standard scaling otherwise), fit exclusively on the training subset of each fold.
- **Loss function**: Huber loss ($\delta = 0.75$) on normalized values.
- **Optimizer**: AdamW (initial $\mathrm{lr} = 1\times10^{-4}$, weight decay $= 5\times10^{-2}$), batch size $= 32$ graphs.
- **Scheduling**: Learning rate halved after 10 epochs without improvement (minimum $1\times10^{-6}$); early stopping after 25 epochs without improvement; maximum 500 epochs.
- **Hardware**: NVIDIA GeForce RTX 5070 Mobile.
### Evaluation metrics
- **$R^2$**: Coefficient of determination.
- **Pearson correlation ($r$)**: Linear correlation.
- **$\mathrm{MAE}$**: Mean absolute error, converted to $\text{kcal/mol}$ via $\Delta G = -RT\ln(10) \cdot \mathrm{p}K_{\mathrm{d}}$.
...
|---|---|
| $R^2$ | $0.38 \pm 0.10$ |
| Pearson correlation ($r$) | $0.63 \pm 0.06$ |
| $\mathrm{MAE}$ | $1.37 \pm 0.10$ kcal/mol |

<img src="https://github.com/msanchezliveros/protein-dna-binding-affinity-gnn/blob/main/figures/F2_scatter_predictions.png?raw=true" alt="Predicted vs experimental affinity" width="500"/>
...
* `<complex_resolution>` — experimental resolution of the structure ($\text{\AA}$).
* `<complex_temperature>` — experimental temperature of the structure ($\text{K}$).
...
The reported value is the mean predicted $\mathrm{p}K_{\mathrm{d}}$ across the five folds, and its standard deviation reflects the ensemble's internal disagreement rather than a calibrated confidence interval.
